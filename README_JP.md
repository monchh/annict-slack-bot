...
Serviceを作成しsystemdを実行することで自動起動できるようにします。ユニット定義ファイルの例は下記になります。

```
# /etc/systemd/system/annict-slack-golang-bot.service
[Unit]
Description=Annict Slack Bot Service
After=network.target

[Service]
WorkingDirectory=${directory_path}$/annict-slack-bot
ExecStart=${directory_path}/annict-slack-bot/annict-slack-bot

Restart=always
RestartSec=5s

EnvironmentFile=${directory_path}/.env
